...
import {doctors, recipes, categories, ingredients} from './dataArray';

export function getDoctorNameById(doctorId) {
  let name;
  doctors.map(data => {
    if (data.id == doctorId) {
      name = data.name;
...
export function getDoctorJobById(doctorId) {
  let job;
  doctors.map(data => {
    if (data.id == doctorId) {
      job = data.job;
...
export function getDoctoraddressById(doctorId) {
  let address;
  doctors.map(data => {
    if (data.id == doctorId) {
...
export function getDoctorPhotoById(doctorId) {
  let photo;
  doctors.map(data => {
    if (data.id == doctorId) {
      photo = data.photo;
